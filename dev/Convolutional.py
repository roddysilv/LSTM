...
def make_timeseries_regressor(window_size, filter_length, nb_input_series=1, nb_outputs=1, nb_filter=4):
    ...
    model = Sequential((
        ...
        Convolution1D(kernel_size=nb_filter, filters=filter_length, activation='linear', input_shape=(window_size, nb_input_series)),
        MaxPooling1D(),     # Downsample the output of convolution by 2X.
        Convolution1D(kernel_size=nb_filter, filters=filter_length, activation='linear'),
        MaxPooling1D(),
        Flatten(),
...
def evaluate_timeseries(timeseries, window_size):
    ...
    model.fit(X_train, y_train, epochs=5000, batch_size=5000, validation_data=(X_test, y_test))
...
# path = '../LSTM/data/residential/daily/Residential_3_daily_sum.csv'
...
df = df[['energy_kWh','temperature','dc_output','humidity']]
...
pred = evaluate_timeseries(timeseries, window_size)
