...
def PlotDaily(arquivo,i,save=False):
    ...
    if save:
        daily_df.to_csv('../data/daily/Residential_' + str(i) + '_daily_sum.csv')
    
    plt.figure()
    plt.plot(daily_df['energy_kWh'],'k')
    plt.title('Soma Diária de Energia - Casa '+str(i))
    plt.xticks(rotation=45)
    plt.savefig('../data/plots/Residential_'+str(i)+'_daily_sum.svg')
    plt.show()
...
for i in range(1,29):
    PlotDaily('../data/separated_by_house_type/all/Residential_'+str(i)+'_treated.csv',
              i,
              save)
    
# =============================================================================
# PlotDaily('csv/all/Residential_19_treated.csv')
# df = pd.read_csv('csv/all/Residential_19_treated.csv',
#                  header=0,
#                  infer_datetime_format=True,
#                  parse_dates=['date'],
#                  index_col=['date'])
# 
# holidays = pd.read_csv('csv/all/Holidays.csv',
#                        header=0,
#                        infer_datetime_format=True,
#                        parse_dates=['date'],
#                        index_col=['date'])
