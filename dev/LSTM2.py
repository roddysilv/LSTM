...
#%%
#Carregando os dados
dataframe = pd.read_csv('diario/Residential_1_daily_sum.csv',index_col=('date'), engine='python')
...
model = Sequential()
model.add(LSTM(4, activation='linear',batch_input_shape=(batch_size, look_back, input_shape), stateful=True, return_sequences=True))
...
history = model.fit(trainX, trainY, epochs=3, batch_size=batch_size, verbose=2, shuffle=False,validation_split=0.33,callbacks=[es])
